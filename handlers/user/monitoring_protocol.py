...
async def monitoring_protocol(callback: CallbackQuery):
    try:
        # await callback.bot.edit_message_text(
        #     text=callback.message.text + '\nВыбрано "Протокол мониторинга"',
    ...
    await DataInput.input_monitoring_protocol.set()

async def create_new_monitoring_protocol(msg: types.Message, state: FSMContext):
    doc_utils = DocumentUtils(settings.GOOGLE_API_KEY,
                              settings.GOOGLE_APP_NAME)
    try:
        sessionId = int(msg.text)
        path = doc_utils.CreateMonitorReport(settings.SPREADSHEET_ID,
                                             sessionId)
